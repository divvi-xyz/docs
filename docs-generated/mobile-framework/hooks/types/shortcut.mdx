...
> **Note**
> The UI/UX for shortcut hooks in Divvi apps is currently focused on "earn" and "claim rewards" use cases and requires a [position pricing hook](./position) to be implemented as well. We plan to expand them to other use cases in the future (shortcuts with custom inputs, not linked to positions, or across dapps).
...
Once the shortcut is defined, the Divvi app needs to know when to show it to the user, by linking it to an existing [position](./position).
...
The [hooks live preview](../live-preview) mode in a Divvi app is the easiest way to test your shortcut hook while developing it.
...
<EditPage editUrl="https://github.com/divvi-xyz/hooks/edit/main/docs/types/shortcut.md" lastModified="2025-02-19T14:27:25.000Z" />